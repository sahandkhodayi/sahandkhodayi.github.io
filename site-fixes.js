...
(function(){
  ...
  style.textContent = `
    /* Never show browser-default link underlines in the terminal UI. */
  ...
  function textPolish(root){
    ...
    root.querySelectorAll('a,button').forEach(el=>{
      ...
      // Turn accidental snake_case labels into readable UI copy, but never touch hrefs.
      if(el.children.length===0 && /lab_[a-z_]+/i.test(el.textContent)){
  ...
  function activationCard(){
    ...
    wrap.innerHTML=`<h3>activation explorer</h3><p>Move x and see what a neuron gets after the activation function.</p>
      <div class="lab-control"><select id="ufn" class="lab-btn"><option value="relu">ReLU</option><option value="sigmoid">sigmoid</option><option value="tanh">tanh</option></select><input id="ux" type="range" min="-6" max="6" step="0.1" value="1"><span id="uxv" class="lab-value">1.0</span></div>
      <div id="uout" class="lab-output"></div>`;
  ...
  function descentCard(){
    ...
    wrap.innerHTML=`<h3>gradient descent</h3><p>Find the minimum of a simple quadratic. Change the learning rate and run the optimizer.</p>
      <div class="lab-control"><span>η</span><input id="dlr" type="range" min="0.01" max="0.45" step="0.01" value="0.12"><span id="dlrv" class="lab-value">0.12</span></div>
      <button id="drun" class="lab-btn">run 30 steps</button><div class="lab-output" id="dout">x = 4.00 · loss = 16.00</div><div class="lab-bar"><i id="dbar"></i></div>`;
    ...
    run.onclick=()=>{let x=4, eta=+lr.value; for(let i=0;i<30;i++) x-=eta*2*x; const loss=x*x; out.textContent=`x = ${x.toFixed(4)} · loss = ${loss.toFixed(6)}`; bar.style.width=Math.max(0,Math.min(100,100*(1-Math.min(1,loss/16))))+'%'};
    return wrap;
  }

  function matrixCard(){
    const wrap=document.createElement('article'); wrap.className='lab-card';
    wrap.innerHTML=`<h3>matrix multiplication</h3><p>Change the inputs and watch a 2×2 matrix multiply a vector.</p>
      <div class="lab-matrix">${[2,1,1,3].map((v,i)=>`<input data-m="${i}" value="${v}">`).join('')}</div>
      <div class="lab-control"><span>x</span><input id="mx1" type="number" value="2" step="1"><input id="mx2" type="number" value="1" step="1"></div>
      <div id="mout" class="lab-output"></div>`;
  ...
  function mountLab(){
    ...
    const grid=section.querySelector('.lab-grid'); grid.append(descentCard(),activationCard(),matrixCard());
    app.prepend(section); textPolish(section);
  }

  function refresh(){ textPolish(document); mountLab(); }
