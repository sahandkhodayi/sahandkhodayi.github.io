/* ============================================================
   LARPSOCIETY — shutdown / poweroff / halt / reboot (purple)
   ============================================================ */
(function(){
  ...
  style.textContent = `
    ...
    #shutdown-overlay.phase-line .sd-line{
      opacity:1;
      animation:sdLineCollapse .7s cubic-bezier(.4,0,.6,1) forwards;
    }
    ...
    .sd-dot{
      ...
      box-shadow:
        0 0 20px #a855f7,
        0 0 60px #a855f7,
        0 0 120px rgba(168,85,247,.9);
      border-radius:50%;
      opacity:0;
      transform:scale(0);
      z-index:3;
    }
    #shutdown-overlay.phase-dot .sd-dot{
      animation:sdDotFade 1.15s ease-out forwards;
    }
    @keyframes sdDotFade{
    ...
    .sd-text{
      ...
      text-shadow:
        0 0 8px #a855f7,
        0 0 24px rgba(168,85,247,.6),
        -1px 0 0 rgba(255,45,120,.4),
         1px 0 0 rgba(0,180,255,.4);
      opacity:0;
    ...
    #shutdown-overlay.phase-text .sd-text{
      opacity:1;
      transform:none;
    }
    .sd-title{
      ...
      text-shadow:
        ...
        -1px 0 0 rgba(255,45,120,.45),
         1px 0 0 rgba(0,180,255,.45);
    }
    .sd-rule{
      width:380px;
      max-width:80vw;
    ...
    .sd-line-txt b{
      color:#ede9fe;
      font-weight:normal;
      text-shadow:0 0 6px rgba(168,85,247,.7);
    }
    .sd-prompt{
      margin-top:44px;
      font-size:12px;
      color:#4c1d95;
      letter-spacing:.24em;
      animation:sdBlink 1.1s steps(2) infinite;
    }
    @keyframes sdBlink{ 50%{ opacity:.15; } }

    @media (prefers-reduced-motion: reduce){
      #shutdown-overlay .sd-line,
      #shutdown-overlay .sd-dot{
  ...
  overlay.innerHTML = `
    ...
    <div class="sd-text">
      ...
      <div class="sd-line-txt">reason <b>manual shutdown</b></div>
  ...
  function onClickWhileOffline(e){
    if (!shuttingDown) return;
    e.preventDefault();
    e.stopPropagation();
    reboot();
  }
  function onTouchWhileOffline(e){
    if (!shuttingDown) return;
    e.preventDefault();
    e.stopPropagation();
    reboot();
  ...
  function armReboot(){
    document.addEventListener('keydown', onKeyWhileOffline, true);
    document.addEventListener('click', onClickWhileOffline, true);
    document.addEventListener('touchstart', onTouchWhileOffline, {capture:true, passive:false});
  }

  function startUptime(){
    const el = () => document.getElementById('sd-uptime');
